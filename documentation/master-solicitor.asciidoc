...
When started with the command line option `-esb` _Solicitor_ will store a CycloneDX JSOM SBOM which describes the Executable JAR as `solicitor.cdx.json` in the current directory.

IMPORTANT: As of now, the license information given in the SBOM `solicitor.cdx.json` is purely based on the package metadata of the included libraries. It does not take into account any (Solicitor based) corrections which are possibly applied to produce `solicitor_licenseinfo.html`. For licensing information take `solicitor_licenseinfo.html` as reference.
...
* `OSS specific` (for license type `OSS-OTHER`) - a nonstandard OSS license which could not be mapped to a SPDX-Id
* `PublicDomain` (for license type `OSS-OTHER`) - any form of public domain which is not represented by an explicit SPDX-Id
* `Unchosen` (for license type `IGNORE`) - this is an unchosen alternative license in a multilicensing scenario.
* `Ignored` (for license type `IGNORE`) - license will be ignored due to other reasons
* `NonOSS` (for license type `COMMERCIAL`) - commercial license, not OSS
...
* An XLSX or XLS (Excel 97) workbook or CSV file which defines the decision table data.
...
** File suffix needs to be _xlsx_ or _xls_.
...
is true if the property of the model is identical to the given value in the Excel (or CSV) sheet.
...
* If the given value of the Excel (or CSV) field starts with the prefix `NOT:` then the outcome of the remaining condition is logically negated, i.e. this field condition is  `true` if the rest of the condition is NOT fulfilled.
...
Decision table data: `LicenseAssignmentV2*.xlsx/xls/csv`
...
Decision table data: `LicenseNameMapping*.xlsx/xls/csv`
...
Decision table data: `MultiLicenseSelection*.xlsx/xls/csv`
...
Decision table data: `LicenseSelection*.xlsx/xls/csv`
...
Decision table data: `LegalPreEvaluation*.xlsx/xls/csv`
...
Decision table data: `LegalEvaluation*.xlsx/xls/csv`
