...
Within the normal workflow `NormalizedLicense` objects are created from `RawLicense` objects via the rules given in the different LicenseAssignment and LicensNameMapping decision tables, see <<Phase 1: Determining assigned Licenses>>. The "raw" license data obtained from Scancode represents licenses either by SPDX-IDs or (if licenses 
...
* If the raw license does not match above criteria or matches a blacklist (see below) then no automatic mapping is done.

==== Blacklist
A blacklist allows suppressing the automatic mapping of specific licenses. The blacklist is configured via a property and is represented by a comma separated list of regular expressions.

The default is:
----
solicitor.scancode.automapping.blacklist=.*unknown.*,.*proprietary.*
----
This prohibits automatic mapping of licenses ids which are ambiguous.  
