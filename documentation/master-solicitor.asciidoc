...
* An Excel (XLSX) table or CSV table which defines the decision table data.

When processing, _Solicitor_ will internally use the rule template to create one or multiple rules for every record found in the Excel (or CSV) sheet. The format (XLSX or CSV) needs to be defined in the _Solicitor_ base configuration in _ruleSource_. The following points are important here:
...
Enabled Decision Tables to be defined as CSV.
