...
Even though these included rules are not "intentionally wrong" they are only samples and *you should never rely on these builtin rules without checking and possibly modifying their content and consulting your lawyer*. _Solicitor_ is a tool
for technically supporting the management of OSS licenses within your project. _Solicitor_ neither gives legal advice nor is a replacement for a lawyer.
...
== Usage
=== Executing Solicitor
_Solicitor_ is a standalone Java (Spring Boot) application. Prerequisite for running it is an existing Java 8 runtime environment. If you do not yet have a the _Solicitor_ executable JAR (`solicitor.jar`) you need to build it as given on the project GitHub homepage https://github.com/devonfw/solicitor .
...
<6> the type of reader; possible values: csv, maven, npm, gradle
...
* The base configuration file, see <<Configuration File>>
* The Drools Rule Templates
* The the builtin decision tables which are referenced in the base configuration, see <<Standard Business Rules>>
...
For the export of the licenses from a maven based project the license-maven-plugin is used.
To configure the plugin some changes needs to be done in the pom.xml, like in the
following example
----
<build>
  <plugins>
...
To generate the input file required for _Solicitor_ the License Plugin needs to be executed
...
* If the given value of the XLS field starts with the prefix `REGEX:` then the remainder of the field defines a https://docs.oracle.com/javase/7/docs/api/java/util/regex/Pattern.html[Java Regular Expression]. For the condition to become true the whole property needs to match the given regular expession.
* If the given value of the XLS field starts with the prefix `RANGE:`  then the remainder of the field defines
a https://maven.apache.org/ref/3.5.0/maven-artifact/apidocs/org/apache/maven/artifact/versioning/VersionRange.html#createFromVersionSpec(java.lang.String)[Maven Version Range]. Using this makes only sense on the artifact
version property.
* If no such prefix is detected, then the behaviour is identical to the normal (verbatim) comparison logic

Fields which are subject to this extended syntact are marked explicitely in the following section.
...
groups to be processed in sequence. When using the builtin base configuration all those decision tables use the internal sample data / rules as contained in _Solicitor_.

To use your own rule data there are three approaches:

* Include your own `rules` section in the project configuration file (so not inheriting from
the builtin base configuration file) and reference your own decision tables there.
...
[source,sql]
----
include::../resources/com/devonfw/tools/solicitor/sql/allden_normalizedlicenses.sql[]
----
...
a deprecation mechanism might be used:
When such a deprecated feature is used _Solicitor_ by default will terminate with an error
message giving information about the deprecation.

By setting the property `solicitor.deprecated-features-allowed` to `true`
(e.g. via the command line, see <<Configuration of Technical Properties>>), the feature
will still be available and only a warning will be logged. The project setup should in any
...
* Reader of type "gradle"  (use Reader of type "gradle2" instead); from Version 1.0.5 on; see https://github.com/devonfw/solicitor/issues/58 
...
----
include::../resources/com/devonfw/tools/solicitor/config/solicitor_base.cfg[lines=23..159]
----
...
----
include::../resources/application.properties[]
----
...
----
include::../resources/samples/application-extension.properties[]
----
