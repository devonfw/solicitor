...
For the Export of the the Licenses from a Maven based Project the license-maven-plugin is used.
To install the Plugin some changes needs to be done in the pom.xml, like following example
----
<build>
	<plugins>
		<plugin>
...
To execute the License Plugin the following commands are used
----
...
Also Maven provides a Depedency plugin which copies all dependencies into the project.
...
"readers" : [ {
      "type" : "maven",
      "source" : "file:$/input/licenses.xml",
      "usagePattern" : "DYNAMIC_LINKING"
   	} ]
----

====== CSV
The CSV input is normally manually generated and should look like this(The csv File is ";" seperated)

[format="csv",width="100%",cols="5", separator=";"]
...
"readers" : [ {
      "type" : "csv",
      "source" : "file:$/input/csvlicenses.csv",
      "usagePattern" : "DYNAMIC_LINKING"
   	} ]
----
...
The Export should look like this(The csv file is "," seperated)
