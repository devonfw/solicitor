...
The _Solicitor_ code and accompanying resources (including this userguide) as stored in the GIT Repository https://github.com/devonfw-forge/solicitor are licensed as Open Source under Apache 2 license (https://www.apache.org/licenses/LICENSE-2.0).
...
== Usage
=== Executing Solicitor
_Solicitor_ is a standalone Java (Spring Boot) application. Prerequisite for running it is an existing Java 8 runtime environment. If you do not yet have a the _Solicitor_ executable JAR (`solicitor.jar`) you need to build it as given on the project GitHub homepage https://github.com/devonfw-forge/solicitor .
...
<9> repoType: Repository to download the sources from: currently possible values: maven, npm
...
----
include::../resources/com/devonfw/tools/solicitor/config/solicitor_base.cfg[lines=22..147]
----
...
The extensions might be provides as JAR file or even as a simple ZIP file. There is only
