...
public class IOHelper {
    ...
    /**
     * Reads text from a given {@link java.io.InputStream} into a String. Uses
     * UTF-8 encoding.
     *
    ...
    /**
     * Assure that the directory in which the given file should be located
     * exists. Try to create the directory if it does not yet exist.
     * 
    ...
    public static void checkAndCreateLocation(File targetFile) {
        ...
        try {
            ...
            try {
                throw new SolicitorRuntimeException("Could not create directory '" + targetDir.getCanonicalPath() + "'",
                        e);
            } catch (IOException e1) {
                // above getCanonicalPath might throw an IOException as well, so
                // falling back to something more robust
                throw new SolicitorRuntimeException("Could not create directory '" + targetDir.getAbsolutePath() + "'",
                        e);
            }
        }
