 ...
 */
@Component
public class FilteredScancodeV32ComponentInfoProvider implements FilteredScancodeVersionComponentInfoProvider {

  private static final Logger LOG = LoggerFactory.getLogger(FilteredScancodeV32ComponentInfoProvider.class);
   ...
   */
  @Autowired
  public FilteredScancodeV32ComponentInfoProvider(ScancodeRawComponentInfoProvider fileScancodeRawComponentInfoProvider,
  ...
  public boolean accept(String toolVersion) {
    ...
    return toolVersion.startsWith("32.");
  }
  ...
  private ScancodeComponentInfo mapScancodeJson(String packageUrl, JsonNode scancodeJson,
    ...
    if (!accept(toolVersion)) {
      throw new ComponentInfoAdapterException("Can only handle version 32 but found version was '" + toolVersion + "'");
    }
    ...
    Map<String, JsonNode> licenseReferencesMap = createLicenseReferencesMap(scancodeJson);
    ...
    for (JsonNode file : scancodeJson.get("files")) {
      ...
      for (JsonNode ld : file.get("license_detections")) {
  ...
  /**
   * Creates a map which holds the license references information.
   *
   * @param scancodeJson the parsed scancode json
   * @return the map of license references from the scancode result. The key of the map is the spdx_license_key.
   */
  private Map<String, JsonNode> createLicenseReferencesMap(JsonNode scancodeJson) {

    Map<String, JsonNode> result = new HashMap<>();
    for (JsonNode licenseReference : scancodeJson.get("license_references")) {
      String spdxLicenseKey = licenseReference.get("spdx_license_key").asText();
      result.put(spdxLicenseKey, licenseReference);
    }
    return result;
  }
   ...
   * @param ma a {@link JsonNode} representing the matches finding data that includes information such as the matched
   *        rule identifier, matched text, and SPDX license key.
   * @param spdxId the spdxId of a single license or Exception
   ...
   *         no change.
   */
  private LicenseCuration.NewLicenseData getEffectiveLicenseInfoWithCuration(String path, JsonNode ma, String spdxId,
    ...
    String ruleIdentifier = ma.get("rule_identifier").asText();
    String matchedText = ma.get("matched_text").asText();
